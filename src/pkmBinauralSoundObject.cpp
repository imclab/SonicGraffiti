...
{
	if (!bInitialized) {
		printf("[ERROR]: Call fftInitialize() first!\n");
		std::exit(1);
		return;
	}
	bNoFilter = true;
	...
	tree.initializeMember();
	bThisObjInitialized = true;
}

// midi-rate
// assumes you are inside the sphere
void pkmBinauralSoundObject::updateNearestFilters(double x, double y, double z)
{
	if (x == 0 && y == 0 && z == 0) {
		bNoFilter = true;
	}
		...
		bNoFilter = false;
		tree.updateNearestFilters(x,y,z);
	}
}
...
void pkmBinauralSoundObject::binauralize(float *signal, float *lOutput, float *rOutput)
{
	...
	// ********************** Left Channel **********************
	float preserveIRNyq = tree.l_ir_fft.imagp[0]; 
	float preserveSigNyq = signal_fft.imagp[0];
	tree.l_ir_fft.imagp[0] = 0;  
	signal_fft.imagp[0] = 0; 
	
	// convolution of complex split data
	vDSP_zvmul(&signal_fft, 1, &tree.l_ir_fft, 1, &l_filtered_signal_fft, 1, fftSize, 1); 
	
	l_filtered_signal_fft.imagp[0] = preserveIRNyq * preserveSigNyq; 
	tree.l_ir_fft.imagp[0] = preserveIRNyq; 
	...
	// ********************** Right Channel **********************
	preserveIRNyq = tree.r_ir_fft.imagp[0]; 
	tree.r_ir_fft.imagp[0] = 0; 
	signal_fft.imagp[0] = 0; 
	
	// convolution of complex split data (both magnitudes and phase)
	vDSP_zvmul(&signal_fft, 1, &tree.r_ir_fft, 1, &r_filtered_signal_fft, 1, fftSize, 1); 
	
	r_filtered_signal_fft.imagp[0] = preserveIRNyq * preserveSigNyq; 
	tree.r_ir_fft.imagp[0] = preserveIRNyq; 
