...
public:
    ...
    void addSound(string filename, 
				  ...
				  double lon = 0.0)
    {
        addedSoundLUT[idx] = soundLibrary.size();
        pkmGPSSoundObject *soundObject = new pkmGPSSoundObject(sample_rate, window_size);
		soundObject->encodeAudio(filename);
		soundObject->setMyAbsolutePosition(pos_x, pos_y, pos_z);
		soundObject->setMyAbsoluteGPS(lat, lon);
		soundLibrary.push_back(soundObject);	
    }
    ...
    {
        // go through the LUT of sound elements, and since
        // we are deleting one of the indexes in soundLibrary, decrement
        // all sound indexes above the input index.  the map holds
        // the mapping then for the revised sound's index, and we use this
        // only when deleting, as the user will know which sound to delete 
        // only by this rule, since they also added the sound in that order
        // but don't have to keep track of which sounds they deleted.
        map<int, int>::iterator itr;
        for(itr = addedSoundLUT.begin(); itr != addedSoundLUT.end(); ++itr){
            if(itr->first > i)
            {
                (itr->second)--;
            }
        }
        soundLibrary[addedSoundLUT[i]]->markForDeletion();
    }
	
	void setListenersAbsolutePosition(double x, 
							 double y, 
							 double z,
	...
	void audioRequested()
	{
        ...
        vector<pkmGPSSoundObject *>::iterator soundToDelete;
		...
		for(vector<pkmGPSSoundObject *>::iterator it = soundLibrary.begin(); it != soundLibrary.end(); it++)
		{
            ...
            if ((*it)->isMarkedForDeletion())
            {
                vDSP_vmul((*it)->getBinauralL() + window_size - pkmAudioWindow::rampOutLength, 1, 
                          pkmAudioWindow::rampOutBuffer, 1, 
                          (*it)->getBinauralL() + window_size - pkmAudioWindow::rampOutLength, 1, 
                          pkmAudioWindow::rampOutLength);
                vDSP_vmul((*it)->getBinauralR() + window_size - pkmAudioWindow::rampOutLength, 1, 
                          pkmAudioWindow::rampOutBuffer, 1, 
                          (*it)->getBinauralR() + window_size - pkmAudioWindow::rampOutLength, 1, 
                          pkmAudioWindow::rampOutLength);
                soundToDelete = it;
                bNeedsDeletion = true;
            }
		...
		if (numAudibleSounds > 0) 
		{
            ...
            vDSP_vclr(currentBuffer, 1, window_size*2);
			
			float *bufferPtr = currentBuffer;
            
            for(int s = 0; s < numAudibleSounds; s++)
            {
                vDSP_vadd(bufferPtr + 0, 2, leftChannels[s], 1, bufferPtr + 0, 2, window_size);
                vDSP_vadd(bufferPtr + 1, 2, rightChannels[s], 1, bufferPtr + 1, 2, window_size);
            }
        ...
        if(bNeedsDeletion)
		{
            ...
            soundLibrary.erase(soundToDelete);
        }
